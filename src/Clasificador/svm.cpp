...
// 				fprintf(stderr, "\n");
		...
		++iter;
// 		fprintf(stderr, "%3.1f%%\r", 100.*iter/l);
// 		fprintf(stderr, "%i\t%i\t%3.1f%%\r", iter, l, 100.*iter/l);
...
svm_model *svm_train(const svm_problem *prob, const svm_parameter *param)
{
	   ...
	   param->svm_type == NU_SVR)
	{
		...
		for(i=0;i<prob->l;i++)
			...
			{
				model->SV[j] = prob->x[i];
// 				printf("o patron %i e VS\n", i);  // Manolo Delgado: 19/07/2007
	...
	else
	{
		...
		for(i=0;i<l;i++) {
			if(nonzero[i]) {
				model->SV[p] = x[i];
// 				printf("o patron %i e VS\n", perm[i]);  // Manolo Delgado: 20/06/2007
				p++;
			}
		}

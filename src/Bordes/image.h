...
/* Struct describing candidates to edges. (Eva Cernadas) */
typedef struct {
  	unsigned int n; /* number of elements */
	unsigned int *x, *y;  /* x and y positions in the image */
  	float *dx, *dy, *grad, *theta; /* gradient components, norme and orientation */
} osl_edgel_chain;

/* Struct describing  edges. (Eva Cernadas) */
typedef struct {
	int id; /* Identificador */
  	unsigned int n; /* number of elements */
	unsigned int *x, *y;  /* x and y positions in the image */
  	float *grad, *theta; /*norme and orientation */
	Point start, end;   /* inicial and end points of the edge */
} osl_edgel_node;



/* Struct for hysteresis step in canny filter. (Eva Cernadas) */
struct list_c{
  int to;
  struct list_c *nextl;
};
typedef struct list_c osl_LINK;
...
osl_edgel_chain* alloc_osl_edgel_chain(unsigned int n);

void free_osl_edgel_chain ( osl_edgel_chain * chain );

osl_edgel_chain * Extract_Gradient_Maxima_2D_edgels ( void *bufferIn,
			     bufferType typeIn,
			     const double low, const double high,
			   	 double *low_val, double *high_val, /* Hysteresis threshold  */
			     float *bufferOutNMS,
			     int *bufferDims,
			     int *borderLengths,
			     float *filterCoefs,
			     recursiveFilterType filterType );
int Extract_Gradient_Theta_Maxima_2D ( void *bufferIn,
			     bufferType typeIn,
			     void *bufferOutGrad,
				 void *bufferOutTheta,
			     void *bufferOutNMS,
...
int   Extract_Gradient_Maxima_2D (void *bufferIn, bufferType typeIn, void *bufferOut, bufferType typeOut, int *bufferDims, int *borderLengths, float *filterCoefs, recursiveFilterType filterType);

int RecursiveFilterOnBuffer ( void *bufferIn, bufferType typeIn, void *bufferOut, bufferType typeOut, int *bufferDims, int *borderLengths, derivativeOrder * derivatives, float *filterCoefs, recursiveFilterType filterType );

void Remove_Gradient_NonMaxima_Slice_2D ( float *maxima, float *gx, float *gy, float *norme, int *bufferDims );
void GradientModulus2D ( float *gradient_modulus,
		    float *derivative_along_X,
		    float *derivative_along_Y, int length );
void GradientOrientation2D ( float *gradient_orientation,
		    float *derivative_along_X,
		    float *derivative_along_Y, int length );
...
/* tools.c */
Point *calc_obj_centroid_contour ( const PointList * cont );
int point_inside(const PointList * list, const Point * p);
int point_inside_pt(const PointList * list, const int col, const int row);
int point_inside_box(const Box box, const int col, const int row);
Box computeBoundingBox(const PointList *p);
double  distanceMaxBetweenPts(const PointList *p);
int  overlapingTest(const PointList *p, const PointList *q,  const int n);
PointList *  unionContour( const PointList *p, const PointList *q);
double  lengthestSide( const Box  *box);
PointList *  copyPointList( const PointList *p);
double minDistance(const PointList * list, const int col, const int row);
int  overlapCells(const PointList **p, const int num_cells, const PointList *q,  const int n);


/* textura_grey.c */
double *texture_features_glrls_grey(const Image *img, Image *img_lab, PointList * in,  int dmax, int a);
double *texture_features_nglds_grey(const Image *img, Image *img_lab, PointList * in, int d, int a);
double *texture_features_haralick_grey(const Image *img, Image *img_lab, PointList * in, int d);
double *texture_features_statistics_grey(const Image *img, Image *img_lab,  PointList * in);

/* edges_govocitos */
PointList **detect_edge_multiscalar_canny ( const Image * in_img,  int *num_edges, int min_dim, int max_edges);
PointList *edge_approach(const PointList * in, const PointList * hull, const int min_dim);
PointList *edge_approach2(const PointList * in, const PointList * hull);

/* scan_fill.c */
double computeRoundness(const PointList * in, const Box box);
Image * computeBinImage(PointList * in, int num_rows, int num_cols);
